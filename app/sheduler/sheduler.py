...
from app.services.services import send_message_to_admin, get_birthday_message
...
@with_session
async def scheduled_task_send_message_to_admin(
    bot: Bot, session: AsyncSessionLocal
) -> None:
    """
...
@with_session
async def reset_can_receive_messages(session: AsyncSessionLocal) -> None:
    """
...
@with_session
async def send_a_birthday_greeting(bot: Bot, session: AsyncSessionLocal) -> None:
    users_with_groups = await user_crud.get_users_and_groups_by_today_birthdays(session)
    for user, group_id in users_with_groups:
        await bot.send_message(
            chat_id=group_id,
            text=get_birthday_message(
                user.user_name if user.user_name != 'Unknown' else user.first_name
            ),
